...
class Conv1DModel(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        x = self.pool(F.relu(self.conv1d(x))) # compress to one convolution block 
        x = self.flatten(x)
        x = F.relu(self.linear(x)) # ReLu is not linear. At least one non-linear to recognize non-linear pattern
...
# Example Input
example_input = torch.randn(1, 1, 100)  # [batch_size, input_channels, seq_length]
