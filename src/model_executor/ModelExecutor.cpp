...
#define METHOD_ALLOCATOR_POOL_SIZE (1 * 16)
...
unsigned int ModelExecutor::getNumberOfInputValues(Result<torch::executor::Method>& method){
    const torch::executor::EValue input_new = method->get_input(0);
    Tensor te = input_new.payload.as_tensor;

    return static_cast<unsigned int>(te.numel());
}
